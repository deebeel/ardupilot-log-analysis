...
MAVProxy через `--mavproxy-args "--load-module keyboard_adapter"` — ідіоматичний спосіб
розширювати MAVProxy (так само map/console), без власного UDP-конекту й очікування
heartbeat. Клавіатурний бекенд — `evdev` (kernel `/dev/input`, однаково на Xorg і
Wayland — дивись розділ нижче), автовизначення пристрою; `Ctrl+C` зупиняє MAVProxy, а
разом з ним і клавіатурний тред (`unload()` модуля).
