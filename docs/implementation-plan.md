...
- Tailwind — через `@astrojs/tailwind`; жодних CDN-стилів (навіть маючи підтверджений вихід в інтернет на VPS — не залежати від живого зовнішнього CDN у рантаймі звіту), все в бандлі.
...
відпрацював без помилок. Конфіги wg0.conf пишуться скриптами (`vps/setup-wireguard.sh` + `local/setup-wireguard.sh`), не з прикладів.
...
- **Docker на VPS — знято.** Живцем перевірено на реальному VPS (`root@1.zdc.space`): вихід в
  інтернет є (apt-репозиторії доступні), тож `provision.sh` ставить Docker Engine з офіційного
  apt-репозиторію сам, без потреби в передвстановленому Docker чи уточненні в замовника.
- **ACME.** Сертифікат все одно надає замовник готовим (тікет), не через Let's Encrypt — тож
  ACME/outbound-залежність тут нерелевантна незалежно від факту наявності інтернету.
