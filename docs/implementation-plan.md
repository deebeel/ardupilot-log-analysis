...
- **Оновлення списку — htmx-поллінг раз на секунду**, а не push/SSE (стійкіший: не треба
  тримати з'єднання живим і переперепідключати). Обгортка `FlightListPoller.astro`
  (`hx-get="/partials/flight-list"`, `hx-trigger="every 1s"`, `hx-swap="outerHTML"`) —
  та сама обгортка з `hx-*` МАЄ бути частиною відповіді партиала `/partials/flight-list`,
  інакше після першого свапу атрибути зникнуть разом зі старим елементом і поллінг
  зупиниться на одному оновленні. `htmx.org` імпортується в `<script>`-тезі шаблону
  (не у frontmatter — той код виконується під час SSR і падає на браузерних глобалах типу
  `location`), вендориться у клієнтський бандл, без CDN.
