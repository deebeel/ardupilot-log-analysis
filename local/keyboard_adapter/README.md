...
## Коли реально керує клавіатура

Тред `keyboard_adapter` живе лише коли ОДНОЧАСНО виконані три умови
(`mavproxy_glue.should_control()`): апарат **озброєний**, у **`FBWA`**
(єдиний режим, для якого це включено — свідомо звужено, без переліку "усіх
ручних" режимів і без toggle-клавіш для призупинки під час друку команд у
`MAV>` — простіше й передбачуваніше), і пілот **явно ввімкнув адаптер**
командою в консолі MAVProxy:

```
kb on    # ввімкнути клавіатурне керування (за замовчуванням — вимкнено)
kb off   # вимкнути
```

Третя умова — свідомий запобіжник: `FBWA + armed` самі по собі можуть настати
ще на землі перед стартом чи випадково під час рулювання, а керування
клавіатурою пілот має включати сам, не воно саме по собі за фактом
режиму/арму. Вихід із `FBWA`, дизарм (навіть без зміни режиму — `disarm` у
самому `FBWA` лишає `flightmode` тим самим, лише знімає armed) чи `kb off` —
будь-яка з трьох умов, що стала хибною, однаково зупиняє керування. Модуль
зіставляється з `self.master.flightmode` і `self.master.motors_armed()`, які
`pymavlink` сам оновлює на кожен `HEARTBEAT` — нічого не парситься вручну;
`kb on`/`kb off` перераховує стан одразу, не чекаючи наступного HEARTBEAT.

На вхід у стан "керувати" модуль перевіряє, чи є `evdev`-клавіатура: якщо
нема — пише про це в консоль MAVProxy (`keyboard_adapter: клавіатура не
знайдена — ручне керування вимкнено`) і НЕ падає. Перевірка відбувається
рівно в момент зміни режиму/арм-стану/`kb`-прапорця, не через окремий
hot-plug watcher — підключення клавіатури ПОСЕРЕДИНІ вже активного керування
навмисно нічого не запускає (до наступного входу). Це свідомо: інакше
довелось би безперервно опитувати наявність пристрою під час польоту, а щойно
фізично підключений USB може "дзвонити" контактами — `EvdevSource.start()`
додатково скидає буфер пристрою перед початком читання, щоб цей дзвін не
трактувався як реальні натискання.
...
| `Up` / `Down` | throttle | `z` | **утримується** (без spring-return) |

Тротл свідомо на `Up`/`Down`, не на `Shift`/`Ctrl`: `evdev` читає клавіші сиро,
незалежно від фокуса вікна (розділ вище), тож `Shift`/`Ctrl` як модифікатори під
час друку команд у `MAV>` (велика буква в назві режиму, `Ctrl+C`) заодно рухали
б тротл. `Up`/`Down` у звичайному тексті команд не з'являються — конфлікту
нема без жодних toggle-клавіш чи іншої логіки.
...
`pyproject.toml`) — це не проблема, бо стенд там і не запускається.
...
│                        # кооперативна зупинка, should_control() (FBWA + armed)
│                        # — без залежності від пакета MAVProxy
