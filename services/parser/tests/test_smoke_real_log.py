...
from parser.models import MAX_SERIES_POINTS, write_json_atomic
...
SERIES_CHANNELS = ("t", "roll", "pitch", "att_roll", "att_pitch")


def test_real_log_parses_into_a_result(real_result):
...
def test_real_result_matches_the_json_contract(real_result):
    # Arrange
...
def test_real_result_is_json_serializable_without_nan(real_result, tmp_path):
    # Arrange
...
def test_real_log_duration_is_within_a_plausible_range(real_result):
    # Arrange
...
def test_real_log_has_at_least_one_manual_phase(real_result):
    # Arrange
...
def test_analyzed_duration_does_not_exceed_total_duration(real_result):
    # Arrange
...
def test_corrections_per_min_is_finite_and_plausible(real_result, axis):
    # Arrange
...
@pytest.mark.parametrize("axis", ["roll", "pitch", "yaw"])
def test_mean_amplitude_is_within_normalized_range(real_result, axis):
    # Arrange
...
@pytest.mark.parametrize("axis", ["roll", "pitch", "yaw"])
def test_oscillation_share_is_a_fraction(real_result, axis):
    # Arrange
...
@pytest.mark.parametrize("axis", ["roll", "pitch", "yaw"])
def test_mean_jerk_is_finite_and_non_negative(real_result, axis):
    # Arrange
...
def test_series_channel_is_downsampled_within_the_point_limit(real_result, channel):
    # Arrange
...
def test_all_series_channels_share_the_same_length(real_result):
    # Arrange
...
def test_default_thresholds_are_embedded_for_the_client(real_result):
    # Arrange
...
def test_amplitude_histogram_edges_exceed_counts_by_one(real_result):
    # Arrange
